"""Postprocess OKVQA benchmark outputs into final CSV tables."""
...
def _load_latest_results_json(output_dir: Path) -> Dict[str, Any]:
    candidates = sorted(output_dir.glob("okvqa_results_*.json"))
    if not candidates:
        raise FileNotFoundError("No okvqa_results_*.json file found.")
    latest = candidates[-1]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Postprocess OKVQA benchmark outputs")
    parser.add_argument("--output_dir", type=str, default="okvqa_results", help="Directory with result JSON")
