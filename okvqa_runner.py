"""Run OKVQA evaluation with existing memory/agent/sleep backend."""
...
from okvqa_postprocessing import save_results_tables
from utils.api_counter import get_api_counters, reset_api_counters


class OkvqaRunner:
    """Benchmark runner for OKVQA data."""
    ...
    def __init__(
        ...
        self.data_dir = Path("OKVQA/preprocessed")
    ...
    def run_table1_evaluation(self, method: str) -> Dict[str, Any]:
        agent = create_agent(method, dataset_name="okvqa", ablation_flags=self.ablation_flags)
        ...
        # Limit per group for practical runtime, but can be increased via num_samples.
        for _, persona_samples in tqdm(grouped.items(), desc=f"OKVQA Table1 {method}"):
            persona_samples = persona_samples[:4]
            ...
            for idx, sample in enumerate(persona_samples):
                ...
                image_meta = sample.get("image_meta", "")

                user_input = query
                if image_meta:
                    user_input = f"{query}\nImage info: {image_meta}"
                ...
                try:
                    ...
                    response = agent.interact(
                        ...
                        importance=0.8,
                        tags=["okvqa", sample.get("question_type", "")],
                        use_memory=(idx > 0),
                    )
        ...
        avg_resp_len = (
            float(np.mean([max(1, len(str(r.get("response", "")).split())) for r in qa_results])) if qa_results else 0.0
        )
    ...
    def run_table2_evaluation(self) -> Dict[str, Any]:
        agent = create_agent("sleep", dataset_name="okvqa", ablation_flags=self.ablation_flags)
        ...
        for _, persona_samples in tqdm(grouped.items(), desc="OKVQA Table2 sleep"):
            ...
            for sample in persona_samples:
                q = sample.get("query", "")
                if sample.get("image_meta"):
                    q = f"{q}\nImage info: {sample.get('image_meta')}"
                try:
                    agent.interact(
                        user_input=q,
                        persona=persona_info,
            ...
            cue_query = "Based on prior visual-question context, what is the best answer?"
    ...
    def run(self) -> Dict[str, Any]:
        ...
        raw_path = self.output_dir / f"okvqa_results_{timestamp}.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run OKVQA benchmark")
    parser.add_argument(
        "--split",
        type=str,
        default="benchmark",
        choices=["benchmark", "val"],
        help="Preprocessed split to use",
    )
    parser.add_argument("--num_samples", type=int, default=400, help="Number of preprocessed samples to evaluate")
    parser.add_argument(
        "--methods",
        nargs="+",
        default=["all"],
    ...
    parser.add_argument("--output_dir", type=str, default="okvqa_results", help="Result output folder")
    parser.add_argument(
        "--ablation",
        type=str,
        default="none",
